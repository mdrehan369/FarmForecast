...
const login = asyncHandler(async (req, res) => {
    ...
    if (!isCorrectPassword(staff.password, password)) throw new ApiError(403, "Wrong Password")

    const accessToken = generateAccessToken(staff, "STAFF")

    staff.password = ""

    return res.status(200).cookie("accessToken", accessToken, options).json(new ApiResponse(200, staff, "LoggedIn"))
})
